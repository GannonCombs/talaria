...
const RESELLER_URL = 'http://127.0.0.1:8787/resy/search';
...
export async function GET(request: NextRequest) {
  ...
  const url = `${RESELLER_URL}?lat=${lat}&long=${long}&day=${day}&party_size=${partySize}`;

  try {
    const res = await paidFetch(url, undefined, {
      service: 'Resy',
      module: 'food',
      endpoint: '/resy/search',
      estimatedCost: 0.001,
    });
    ...
    return NextResponse.json(
      { error: 'Resy search failed', detail: (err as Error).message },
      { status: 500 },
    );
  }
}
